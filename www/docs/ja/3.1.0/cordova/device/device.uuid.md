...
iOSでの`uuid`は、デバイスごとに固有ではありません。インストールごと、アプリケーションごとに異なります。 アプリを削除、再インストールした場合に変更される場合があり、
iOS のバージョンをアップグレード、またはiOSバージョンごとのアプリにアップグレード(iOS 5.1からの変更）された場合にも変更される場合があります。 
iOSでの`uuid`は信頼性の高い値ではありません。
